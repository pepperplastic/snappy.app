...
// ── Session ID (ties all rows from one visit together) ──
...
export default function App() {
  ...
  const reset = () => {
    ...
    // FIX 3: Reset method to empty string (not 'kit')
                          ...
                          if (e.key === 'Enter' && gateEmail.includes('@')) {
                            ...
                            fetch('/api/submit-lead', {
                              ...
                              body: JSON.stringify({
                                ...
                                itemType: analysis?.item_type || '', source: 'variant_b_gate', variant, ...getStoredUtm(), ip: getIP(), sessionId: getSessionId(),
                              }),
                            }).catch(() => {})
                          }
                        }}
                    ...
                    if (e.key === 'Enter' && nudgeEmail.includes('@')) {
                      ...
                      fetch('/api/submit-lead', {
                        ...
                        body: JSON.stringify({
                          ...
                          description: analysis?.description || '', details: analysis?.details || [],
                          offerNotes: analysis?.offer_notes || '', confidence: analysis?.confidence || '',
                          itemType: analysis?.item_type || '', source: 'variant_c_nudge', variant, ...getStoredUtm(), ip: getIP(), sessionId: getSessionId(),
                        }),
                      }).catch(() => {})
                      onGetOffer()
                    }
...
const LEGAL_CONTENT = {
  ...
  privacy: {
    ...
    date: 'Last Updated: February 24, 2026',
    paragraphs: [
    'Welcome to the DW5 LLC d/b/a Snappy Gold website (https://snappy.gold or the "Website"). This written policy (the "Privacy Policy") is designed to tell you about our practices regarding collection, use, and disclosure of information that you may provide via this Website and certain related services.',
    ],
  },
  tos: {
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div>
            <h2 style={{ fontSize: 22, fontWeight: 600, color: '#FAF6F0', margin: 0, fontFamily: "'EB Garamond', serif" }}>{content.title}</h2>
            <p style={{ fontSize: 12, color: '#C8953C', marginTop: 4, fontFamily: "'DM Sans', sans-serif", letterSpacing: '0.04em' }}>{content.date}</p>
          </div>
        ...
        <div style={{ padding: '24px 28px 32px', overflowY: 'auto', flex: 1, WebkitOverflowScrolling: 'touch' }}>
          {content.paragraphs.map((p, i) => (
            <p key={i} style={{ fontSize: 15, color: 'rgba(250,246,240,0.8)', lineHeight: 1.7, marginBottom: 14, fontFamily: "'EB Garamond', serif" }}>{p}</p>
          ))}
        </div>
      </div>
    </div>
...
const styles = {
  app: { minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: '"DM Sans", -apple-system, sans-serif', color: dark, background: cream },
  ...
  modalOverlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 },
  modalContent: { background: '#FFFDF8', borderRadius: 16, padding: 32, maxWidth: 380, width: '100%', position: 'relative', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' },
  modalClose: { position: 'absolute', top: 12, right: 16, background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: muted, padding: 4 },
  contactItem: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 10, border: `1px solid ${border}`, textDecoration: 'none', color: dark, fontSize: 15, transition: 'all 0.2s' },
  contactIcon: { fontSize: 20, width: 24, textAlign: 'center' },
  main: { flex: 1, maxWidth: 960, margin: '0 auto', width: '100%', padding: '0 24px' },
  heroSection: { textAlign: 'center', paddingTop: 44, paddingBottom: 60, position: 'relative', overflow: 'hidden' },
  ...
  heroButtons: { display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 16, flexWrap: 'wrap', position: 'relative', zIndex: 1 },
  heroTitle: { fontFamily: '"Playfair Display", serif', fontSize: 'clamp(36px, 7vw, 64px)', fontWeight: 600, lineHeight: 1.1, letterSpacing: '-0.02em', color: dark, marginBottom: 20, position: 'relative', zIndex: 1 },
  ...
  stepTitle: { fontFamily: '"Playfair Display", serif', fontSize: 18, fontWeight: 600, marginBottom: 6 },
  stepDesc: { fontSize: 14, color: muted, lineHeight: 1.5 },
  centeredSection: { textAlign: 'center', paddingTop: 48, paddingBottom: 64, maxWidth: 600, margin: '0 auto' },
  sectionTitle: { fontFamily: '"Playfair Display", serif', fontSize: 28, fontWeight: 600, marginBottom: 12 },
  sectionSub: { fontSize: 16, color: muted, lineHeight: 1.6, marginBottom: 32 },
  dropZone: { border: '2px dashed', borderRadius: 16, padding: 48, marginBottom: 24, transition: 'all 0.2s', cursor: 'pointer' },
  dropZoneInner: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 },
  dropIcon: { color: goldLight, opacity: 0.7 },
  dropText: { fontSize: 16, fontWeight: 500, color: dark },
  dropSubtext: { fontSize: 14, color: muted },
  captureButtons: { display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 24, flexWrap: 'wrap' },
  captureBtn: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 12, background: `linear-gradient(135deg, ${gold}, ${goldLight})`, color: '#fff', fontSize: 15, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: 'inherit', animation: 'ctaRing 2.2s ease-out infinite' },
  captureBtnSecondary: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 12, background: '#FFFDF8', color: dark, fontSize: 15, fontWeight: 600, border: `1px solid ${border}`, cursor: 'pointer', fontFamily: 'inherit' },
  tipBox: { padding: '14px 20px', borderRadius: 12, background: goldBg, fontSize: 13, color: muted, lineHeight: 1.5 },
  ...
  scanLine: { position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'linear-gradient(90deg, transparent, #22C55E, transparent)', animation: 'scan 2s ease-in-out infinite', boxShadow: '0 0 12px rgba(34, 197, 94, 0.5)' },
  analyzingText: { padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 },
  spinner: { width: 32, height: 32, border: `3px solid ${border}`, borderTop: `3px solid ${gold}`, borderRadius: '50%', animation: 'spin 0.8s linear infinite' },
  analyzingTitle: { fontFamily: '"Playfair Display", serif', fontSize: 22, fontWeight: 600 },
  analyzingSub: { fontSize: 14, color: muted, maxWidth: 360, lineHeight: 1.5 },
  offerBadge: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 100, background: goldBg, color: gold, fontSize: 13, fontWeight: 600, marginBottom: 24 },
  offerBadgeUpdated: { display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 100, background: 'rgba(34, 197, 94, 0.1)', color: '#16A34A', fontSize: 13, fontWeight: 600, marginBottom: 24, animation: 'badgeFlash 1.5s ease' },
  offerCard: { borderRadius: 20, border: `1px solid ${border}`, background: '#FFFDF8', overflow: 'hidden', marginBottom: 28, textAlign: 'left' },
  offerTop: { display: 'flex', gap: 20, padding: 24, flexWrap: 'wrap' },
  offerImageWrap: { display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 },
  offerImage: { width: 120, height: 120, borderRadius: 12, objectFit: 'contain', background: '#F5F0E8' },
  offerThumbRow: { display: 'flex', gap: 4 },
  offerThumb: { width: 38, height: 38, borderRadius: 6, objectFit: 'cover', background: '#F5F0E8', border: '1px solid #E8DCC8' },
  offerInfo: { flex: 1, minWidth: 200 },
  offerTitle: { fontFamily: '"Playfair Display", serif', fontSize: 22, fontWeight: 600, marginBottom: 8 },
  offerDesc: { fontSize: 14, color: muted, lineHeight: 1.6 },
  offerDetails: { borderTop: `1px solid ${border}`, padding: '16px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '8px 24px' },
  detailRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', fontSize: 14, padding: '8px 0', gap: 16 },
  detailLabel: { color: muted, flexShrink: 0, minWidth: 110 },
  detailValue: { fontWeight: 500, textAlign: 'right' },
  detailValueWrap: { display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6, textAlign: 'right' },
  pencilBtn: { background: 'none', border: 'none', cursor: 'pointer', padding: 2, display: 'flex', alignItems: 'center', fontSize: 12, opacity: 0.5, transition: 'opacity 0.2s' },
  weightRow: { animation: 'weightPulse 2s ease-in-out 1s 2', borderRadius: 8, position: 'relative' },
  weightValue: { color: goldLight, fontWeight: 600 },
  weightInfoIcon: { display: 'inline-flex', alignItems: 'center', marginLeft: 6, fontSize: 13, color: goldLight, cursor: 'pointer', position: 'relative' },
  weightTooltip: { position: 'absolute', bottom: 'calc(100% + 8px)', left: '50%', transform: 'translateX(-50%)', background: '#1A1A1A', color: '#fff', fontSize: 12, lineHeight: 1.4, padding: '10px 14px', borderRadius: 10, width: 220, zIndex: 10, textAlign: 'center', boxShadow: '0 4px 16px rgba(0,0,0,0.25)', fontWeight: 400 },
  weightTooltipArrow: { position: 'absolute', bottom: -5, left: '50%', transform: 'translateX(-50%) rotate(45deg)', width: 10, height: 10, background: '#1A1A1A' },
  detailEditInput: { padding: '4px 8px', borderRadius: 6, border: `1px solid ${goldLight}`, fontSize: 14, fontFamily: 'inherit', fontWeight: 500, background: '#FFFDF8', color: dark, outline: 'none', width: '60%', textAlign: 'right' },
  offerRange: { borderTop: `1px solid ${border}`, padding: 24, textAlign: 'center', background: 'linear-gradient(180deg, #FFFDF8, #FDF6E8)' },
  offerRangeLabel: { fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: muted, marginBottom: 8 },
  offerPrices: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16 },
  offerPrice: { fontFamily: '"Playfair Display", serif', fontSize: 36, fontWeight: 700, color: gold },
  offerDash: { fontSize: 24, color: '#D4C5A9' },
  offerNotes: { fontSize: 13, color: muted, marginTop: 12, lineHeight: 1.5, maxWidth: 400, margin: '12px auto 0' },
  correctionSection: { borderTop: `1px solid ${border}`, padding: '16px 24px' },
  correctionToggle: { background: 'none', border: 'none', color: muted, cursor: 'pointer', fontSize: 14, fontFamily: 'inherit', fontWeight: 500, padding: '10px 0', width: '100%', textAlign: 'center', textDecoration: 'underline', textUnderlineOffset: 3, textDecorationColor: '#D4C5A9' },
  correctionForm: { textAlign: 'left' },
  correctionTitle: { fontSize: 15, fontWeight: 600, marginBottom: 10, color: dark, textAlign: 'center' },
  correctionActions: { display: 'flex', gap: 16, justifyContent: 'center' },
  correctionLink: { background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, fontFamily: 'inherit', fontWeight: 500, color: goldLight, padding: '4px 0', textDecoration: 'underline', textUnderlineOffset: 3, textDecorationColor: '#D4C5A9' },
  correctionSub: { fontSize: 13, color: muted, marginBottom: 14 },
  correctionRow: { marginBottom: 10 },
  correctionLabel: { display: 'block', fontSize: 12, fontWeight: 500, color: muted, marginBottom: 3 },
  correctionInput: { width: '100%', padding: '8px 12px', borderRadius: 8, border: `1px solid ${border}`, fontSize: 14, fontFamily: 'inherit', background: '#FFFDF8', color: dark, outline: 'none' },
  reEstimateOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(255, 253, 248, 0.6)', backdropFilter: 'blur(2px)' },
  reEstimateScanLine: { position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'linear-gradient(90deg, transparent, #22C55E, transparent)', animation: 'scan 1.5s ease-in-out infinite', boxShadow: '0 0 16px rgba(34, 197, 94, 0.5)' },
  reEstimateText: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 15, fontWeight: 600, color: '#16A34A' },
  spinnerGreen: { width: 20, height: 20, border: '2.5px solid #E0E0E0', borderTop: '2.5px solid #22C55E', borderRadius: '50%', animation: 'spin 0.8s linear infinite' },
  firmOfferBtn: { display: 'inline-flex', alignItems: 'center', gap: 10, padding: '16px 32px', borderRadius: 12, background: 'linear-gradient(135deg, #16A34A, #22C55E)', color: '#fff', fontSize: 16, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.25s ease', boxShadow: '0 4px 16px rgba(34, 197, 94, 0.3)', animation: 'pulseGreen 2s ease-in-out infinite' },
  offerCaveat: { fontSize: 13, color: muted, marginTop: 12 },
  linkBtn: { background: 'none', border: 'none', color: muted, cursor: 'pointer', fontSize: 14, marginTop: 16, fontFamily: 'inherit', textDecoration: 'underline', textUnderlineOffset: 3 },
  form: { textAlign: 'left' },
  formGroup: { marginBottom: 20 },
  formLabel: { display: 'block', fontSize: 14, fontWeight: 500, marginBottom: 6, color: dark },
  formInput: { width: '100%', padding: '12px 16px', borderRadius: 10, border: `1px solid ${border}`, fontSize: 15, fontFamily: 'inherit', background: '#FFFDF8', color: dark, outline: 'none', transition: 'border-color 0.2s' },
  formDisclaimer: { fontSize: 13, color: muted, marginTop: 14, textAlign: 'center' },
  formRow: { display: 'flex', gap: 12 },
  shippingOptions: { display: 'flex', gap: 12, marginBottom: 20 },
  shippingOption: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '20px 16px', borderRadius: 14, border: `2px solid ${border}`, background: '#FFFDF8', cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit', gap: 6 },
  shippingOptionActive: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '20px 16px', borderRadius: 14, border: `2px solid ${goldLight}`, background: goldBg, cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit', gap: 6, boxShadow: `0 0 0 3px rgba(200,149,60,0.1)` },
  shippingOptionIcon: { fontSize: 28, marginBottom: 4 },
  shippingOptionTitle: { fontFamily: '"Playfair Display", serif', fontWeight: 600, fontSize: 16, color: '#1A1A1A' },
  shippingOptionDesc: { fontSize: 12, color: muted, lineHeight: 1.4 },
  successIcon: { width: 72, height: 72, borderRadius: '50%', background: '#E8F5E8', color: '#2E7D2E', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' },
  successSteps: { textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 32 },
  successStep: { display: 'flex', alignItems: 'center', gap: 14, fontSize: 15 },
  successStepNum: { width: 28, height: 28, borderRadius: '50%', background: goldBg, color: gold, fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  footer: { textAlign: 'center', padding: '32px 24px', borderTop: `1px solid ${border}` },
  footerText: { fontSize: 13, color: muted, marginBottom: 4 },
  footerDisclaimer: { fontSize: 12, color: '#B5A992' },
  footerLinks: { display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' },
  footerLink: { fontSize: 12, color: '#C8953C', textDecoration: 'none', cursor: 'pointer' },
  footerLinkDivider: { fontSize: 12, color: '#555' },
  webcamOverlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 },
  webcamModal: { background: '#FFFDF8', borderRadius: 20, overflow: 'hidden', maxWidth: 520, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' },
  webcamHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: `1px solid ${border}` },
  webcamCloseBtn: { background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: '#9B8E7B', padding: '4px 8px' },
  webcamVideoWrap: { position: 'relative', background: '#000', aspectRatio: '4/3', overflow: 'hidden' },
  webcamVideo: { width: '100%', height: '100%', objectFit: 'cover', display: 'block' },
  webcamLoading: { position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 14 },
  webcamError: { padding: 40, textAlign: 'center', color: '#9B8E7B', fontSize: 14, lineHeight: 1.5 },
  webcamControls: { padding: '20px 20px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  webcamShutter: { width: 64, height: 64, borderRadius: '50%', border: '3px solid #C8953C', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.1s' },
  webcamShutterInner: { width: 50, height: 50, borderRadius: '50%', background: 'linear-gradient(135deg, #1A1A1A, #333)' },
}
